...
export class FreelancerService {
  ...
  async searchActiveProjects(skillIds: number[]): Promise<any> {
    ...
    try {
      this.appService
        .getAppLogger()
        .log(`Searching projects with skills: [${skillIds.join(', ')}] from last 48 hours`);
      ...
      while (hasMore) {
        ...
        try {
          ...
          if (projects.length === 0) {
            ...
            this.appService
              .getAppLogger()
              .log(`Fetched ${projects.length} projects (total: ${allProjects.length})`);
            offset += limit;
          ...
          this.appService
            .getAppLogger()
            .error(`Error fetching batch at offset ${offset}: ${error.message}`);
